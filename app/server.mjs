...
// Windows/部署平台无 macOS 钥匙串，从 .env 读取凭证（gitignore 已忽略该文件）
if (existsSync(path.join(path.dirname(fileURLToPath(import.meta.url)), '.env'))) {
  for (const line of readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), '.env'), 'utf8').split(/\r?\n/)) {
