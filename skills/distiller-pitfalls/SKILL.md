...
## 18. 书签抓到的地址可能是图片地址（正文被存到图片键上）

- **现象**：点「去保存全文」后提示一直停在「等待全文送达…」，卡片不亮、也没有新的学习记录；库里却出现了键是 `https://pic*.zhimg.com/v2-….webp` 的条目，正文内容还是对的
- **原因**：知乎点开图片（灯箱看大图）时**地址栏会变成图片地址，而页面 DOM 仍是那篇文章**。书签读 `location.href` 作为文章地址，于是全文被存到了图片地址这个键上；卡片的等待键来自收藏条目地址，两边永远对不上（ingest 的幂等写入让这个错误键也留了下来）
- **解法**：书签优先取 `link[rel=canonical]` / `meta[property="og:url"]`，读不到才退回 `location.href`；识别到地址是图片地址（`*.zhimg.com`）就直接提示并中止。服务端 `/api/ingest` 也拒绝把图片地址当文章地址（双保险）
- **注意**：书签代码存在用户浏览器的书签里，改动后**必须重新拖一次**才会生效；已存下的错误键条目需要用 `DELETE /api/distilled?url=<键>` 清掉
